...
| --- | ---: | ---: | --- | --- |
| Changed-scope | 8.5/10 | 9.0/10 | Foundation's proven advantage is narrower: stronger repository-path validation and direct routing of the normalized union into safe path-aware or full checks. DeepSeek also disables optional locks and fsmonitor observation, external diff and text conversion, hardens ref resolution, and rejects invalid UTF-8 path evidence; it preserves clearer committed, staged, unstaged, and untracked groups plus exact base, head, and merge-base identities. | Keep Foundation's validated routing boundary; compare future Git hardening against DeepSeek's existing protections rather than assuming a general safety lead. |
