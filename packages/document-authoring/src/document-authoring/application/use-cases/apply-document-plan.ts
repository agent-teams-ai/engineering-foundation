...
async function safelyCancelBeforePublication(
  ...
  try {
    ...
    if (finalDestination.state !== "absent" && !v2MissingParent) {
      return { receipt: await noPublicationReceipt(dependencies.schema, plan, "rejected", { ruleId: "document.transaction.destination-raced", message: finalDestination.state === "exact"
    ...
    return { receipt: await noPublicationReceipt(dependencies.schema, plan, outcome, { ruleId: outcome === "cancelled"
        ? "document.transaction.cancelled"
        : "document.transaction.failed-before-publication", message: errorMessage(failure) }, directoryEvidence === undefined ? {} : { directoryEvidence }) };
  } catch (cleanupError) {
    return {
      cleanupFailure: cleanupError,
      receipt: await recoveryReceipt(dependencies.schema, plan, {
...
async function runApplyTransaction(
  ...
  if (authority.state !== "current") {
    ...
    return noPublicationReceipt(dependencies.schema, plan, "authority-stale", { ruleId: "document.transaction.authority-stale", message: authority.reason }, { phase: "authority" });
  }
  ...
  if (destination.state === "conflict" || destination.state === "unverifiable") {
    state.retainTransactionBarrier = false;
    return noPublicationReceipt(dependencies.schema, plan, "rejected", { ruleId: "document.transaction.destination-conflict", message: destination.reason });
  }
...
export async function applyDocumentPlan(
  ...
  if (request.signal?.aborted === true) {
    return noPublicationReceipt(dependencies.schema, plan, "cancelled", { ruleId: "document.transaction.cancelled", message: "Document transaction was cancelled before coordination." });
  }
