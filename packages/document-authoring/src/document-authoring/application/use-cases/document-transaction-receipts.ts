...
export async function successReceipt(
  ...
  if (plan.schemaVersion === 2) {
    ...
    return createDocumentReceipt(schema, {
      ...receiptBaseV2(
        plan,
...
export async function noPublicationReceipt(
  ...
  detail: { readonly ruleId: string; readonly message: string },
  options: {
  ...
  if (plan.schemaVersion === 2) {
    return createDocumentReceipt(schema, {
      ...
      diagnostics: Object.freeze([diagnostic(options.phase ?? "apply", detail.ruleId, detail.message)]),
      outcome
    }, plan);
  }
  return createDocumentReceipt(schema, {
