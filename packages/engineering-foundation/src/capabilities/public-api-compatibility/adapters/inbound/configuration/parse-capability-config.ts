...
export function readConfigurationHeader(input: unknown): Record<string, unknown> {
  const root = record(input, "public API compatibility config");
  schemaVersion(root["schemaVersion"]);
  return root;
}

export function parseCapabilityConfig(root: Record<string, unknown>): PublicApiCompatibilityPolicy {
