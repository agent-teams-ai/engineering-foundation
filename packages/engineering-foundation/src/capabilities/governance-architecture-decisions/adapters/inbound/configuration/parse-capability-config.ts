...
export function parseCapabilityConfig(input: unknown): ArchitectureDecisionPolicy {
  ...
  for (const adrRoot of adrRoots) {
    assertConfigurationRepositoryPath(adrRoot);
  ...
  assertConfigurationRepositoryPath(acceptedBaselinePath);
