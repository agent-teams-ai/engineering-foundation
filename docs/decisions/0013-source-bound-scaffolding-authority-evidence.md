...
2. The consumer remains authoritative for target ownership, owner document
   identity and path, and the statuses allowed by one Composition. These facts
   are strict data, never callbacks or executable policy.
...
4. The target catalog binds both the owner document ID and its repository path.
   The document is still the source of truth for its status and metadata; the
   catalog owns the package-to-owner binding.
...
6. Configuration, catalog, and owner document assertions are included in the
   complete Plan read set, authority snapshot digest, Authority Evidence digest,
   and Plan digest. Unknown verifiers, versions, fields, and unsafe source paths
   fail closed.
...
- Owner revocation, rebinding, or source mutation invalidates a saved Plan and
  prevents further publication by cooperative Foundation-aware writers.
...
- The canonical scaffolding surface has the approved public API fingerprint
  `sha256:470c7f526844af629d50e7b4f82649e3dd9b0bc22e8e3f07764749e8e904d405`.
