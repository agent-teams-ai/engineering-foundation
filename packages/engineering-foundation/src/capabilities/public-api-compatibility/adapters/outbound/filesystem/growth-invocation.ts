...
export async function assertGrowthDestination(root: string, path: string): Promise<void> {
  ...
  for (const directory of directories) {
    ...
    // The path is assembled from the validated repository-relative config.
    // lstat on every parent is the portable identity check: it rejects links
    // and reparse entries without comparing platform-specific realpath spelling
    // (`/private` on macOS, `\\?\\` and case on Windows).
