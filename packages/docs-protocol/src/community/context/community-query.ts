...
import { createCommunityMiniSearchIndex } from "./minisearch-adapter.js";
...
function projectDocuments(documents: readonly DocsFindDocument[], query: Readonly<DocsFindQueryV3>, advisoryText?: string) {
...
export async function projectCommunityFind(input: {
  ...
  const projection = projectDocuments(complete ? evidence.documents : Object.freeze([]), query, advisoryText);
  return Object.freeze({
