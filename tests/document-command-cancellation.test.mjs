...
async function collectCancellation(signal) {
  const consumerRoot = await mkdtemp(join(tmpdir(), "foundation-docs-cancel-"));
  const child = fork(childPath, [consumerRoot], { execArgv: [], silent: true });
  let stderr = "";
  let stdout = "";
...
for (const signal of ["SIGINT", "SIGTERM"]) {
  test(`${signal} cancels docs doctor through the CLI boundary`, {
    skip: process.platform === "win32",
    timeout: 10_000,
  }, async () => {
    const { result, stderr, stdout } = await collectCancellation(signal);
    ...
    await assertSchema(
      ...
      envelope,
      `docs-doctor-${signal.toLowerCase()}`,
    );
  });
}
