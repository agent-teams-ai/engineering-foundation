import { type DocsCommandEnvelopeV2, type DocsCommandEnvelopeV3 } from "../../contracts/command.js";
...
function renderInfo(result: Record<string, unknown>): readonly string[] {
  ...
  const validators = (result["semanticValidatorIds"] as unknown[] | undefined)?.join(",") ?? "";
  lines.push(`Semantic validators: ${validators.length === 0 ? "none" : validators}`);
  return lines;
}
...
function renderNew(result: Record<string, unknown>): readonly string[] {
  ...
  if (typeof result["planDigest"] === "string") {lines.push(`Plan: ${result["planDigest"]}`);}
