...
const projectedDependencies = Object.freeze([
...
function inputCapture(entry, captured) {
  const state = { bytes: 0 };
  let inputCount = 0;
  return {
    ...
    setup(builder) {
      ...
      builder.onLoad({ filter: /.*/ }, async ({ path }) => {
        ...
        const { bytes } = await readStableRegularFile(path, state, "Markdown bundle input");
        // These are the bytes esbuild consumes, not a post-build reread.
...
async function proveComponents(captured, lock, readArchive) {
...
export async function buildMarkdownDistribution({ packageRoot, sourceLockBytes, readArchive }) {
  ...
  const lock = parse(sourceLockBytes.toString("utf8"));
  ...
  const result = await build({
    ...
    plugins: [inputCapture(entry, captured)],
  });
  assertCapturedInputs(result, captured, physicalRoot);
  const { components, identities } = await proveComponents(captured, lock, readArchive);
...
export function projectMarkdownManifest(manifest, distribution) {
  ...
  for (const name of projectedDependencies) {
