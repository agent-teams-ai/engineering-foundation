...
export async function verifyConsumerTargetLockfile(
  ...
  assertSelectedTargetLockfileBytes(selected, installed.state === "file" ? installed.bytes : undefined,
    "Selected target lock bytes changed during installation or target apply.");
}

export const nodeConsumerTargetLockfileReader: ConsumerTargetLockfileReader =
  Object.freeze({ read: readConsumerTargetLockfile });

export function assertCapturedConsumerTargetLockfile(
  selected: ValidatedTargetLock | undefined, operations: readonly KnownFileTransactionOperationInput[]
): void {
  if (selected === undefined) {return;}
  const captured = operations.find(operation => operation.path === "pnpm-lock.yaml");
  assertSelectedTargetLockfileBytes(selected, captured?.postimage.bytes,
    "Captured target lock differs from the selected preparation bytes.");
}

function assertSelectedTargetLockfileBytes(
  selected: ValidatedTargetLock, observed: Uint8Array | undefined, message: string
): void {
  if (observed === undefined || !Buffer.from(selected.bytes).equals(Buffer.from(observed))) {
    throw new TypeError(message);
  }
  assertQualifiedPnpmLockfileV2(observed, selected.desired);
}
