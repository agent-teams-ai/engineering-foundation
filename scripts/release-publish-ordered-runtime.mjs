...
function releaseNotes(changelog, version) {
  ...
  const contentLines = lines.slice(start + 1);
  const next = contentLines.findIndex((line) => line.startsWith("## "));
  const content = contentLines.slice(0, next < 0 ? undefined : next).join("\n").trim();
  if (content.length === 0) {
