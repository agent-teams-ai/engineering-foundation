import { createVerifiedGrowthObservation } from "./adapters/outbound/reviewrouter/verified-growth-observation.js";
...
import { assertGrowthMetadataRootSources } from "./adapters/outbound/filesystem/growth-metadata-root-sources.js";
...
export interface SdkGrowthAuthorityModuleDependencies {
  ...
  readonly installedInventory?: import("./application/ports/growth-authority.js").GrowthInstalledInventoryReader | undefined;
...
export function createSdkGrowthAuthorityModule(dependencies: SdkGrowthAuthorityModuleDependencies) {
  ...
  if (dependencies.now === undefined) { throw new GrowthObservationInvariantError("growth-authority-clock-required"); }
  ...
  async function setup(input: SdkGrowthAuthorityOperationInput, operation: "check" | "promote-release", admissionReceiptId?: string) {
    ...
    const authority = new ReviewRouterGrowthAuthorityAcl(dependencies.transport, common.fingerprint, now, dependencies.installedInventory);
    ...
    const context = new VerifiedGrowthInputContext({ candidate, authority, binding, operation,
      assertMetadataRoots: (roots, cancellation) => assertGrowthMetadataRootSources(roots, input.consumerRoot, identityInputs, cancellation),
  ...
  return Object.freeze({
    async qualifyCheck(input: SdkGrowthAuthorityOperationInput) {
      ...
      await state.context.read({ ...state.policy.sdkGrowth.comparison, decisionsPath: state.policy.sdkGrowth.decisionsPath }, {
        ...(input.signal === undefined ? {} : { signal: input.signal }),
        throwIfCancelled() { if (input.signal?.aborted === true) { throw input.signal.reason; } }
      });
      const context = {
        read: async () => structuredClone(state.context.resolution().context),
        resolution: () => state.context.resolution(),
        packedCandidates: () => state.context.packedCandidates()
      };
      return qualifySdkGrowth({ consumerRoot: input.consumerRoot, policy: state.policy, invocation: state.invocation,
        ...
        workspace: { read: async () => state.inventory }, context, authority: state.authority,
        observationBoundary: (local) => createVerifiedGrowthObservation(local, state.context, common.fingerprint),
    ...
    async promoteRelease(input: SdkGrowthAuthorityOperationInput & { readonly admissionReceiptId: string }): Promise<{ readonly snapshots: readonly import("./application/model/public-api.js").PublicApiSnapshot[]; readonly receipt: GrowthAuthorityReceipt }> {
      ...
      const provenance = resolved.grant.admissionReceipt.receipt.provenance;
      invariant(provenance !== undefined && provenance.completion.reportByteLength === reportBytes.byteLength,
        "growth-authority-admission-report-mismatch");
      const report = parseFinalizedGrowthReport(reportBytes, common.fingerprint);
      invariant(report.authority.status === "verified" && report.authority.receiptDigest === resolved.grant.admissionReceipt.receipt.grantDigest
        && report.authority.workflowRef === provenance.grant.workflowRef && report.authority.runRef === provenance.grant.runRef,
        "growth-authority-admission-provenance-mismatch");
      ...
      const snapshots = await preflightPublicApiPromotions({ consumerRoot: input.consumerRoot, policy: state.policy.compatibility,
        ...(input.signal === undefined ? {} : { signal: input.signal }) }, [common, artifacts], async (writes) => {
          ...
          // Completion is asynchronous external IO. Its receipt cannot authorize
          // writes against source bytes that changed while it was in flight.
          invariant(growthCanonicalJson(await state.readInvocation(cancellation)) === growthCanonicalJson(state.invocation), "growth-execution-inputs-changed");
        });
