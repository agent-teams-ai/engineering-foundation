...
export async function ensureMutationStateDirectory(
  ...
  try {
    ...
    if (
      ...
      return ensureMutationStateDirectory(consumerRoot);
    }
    throw error;
  }
  await syncMutationStateDirectory(consumerRoot);
  return directory;
}

export async function syncMutationStateDirectory(path: string): Promise<void> {
  try {
 ...
 */
export async function syncMutationStateDirectoryStrictly(
  path: string
): Promise<void> {
  const handle = await open(path, "r");
  try {
...
export async function pruneMutationStateDirectory(
  consumerRoot: string
): Promise<void> {
  try {
